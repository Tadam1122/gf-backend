...
async function priceAMS(link, page) {
  ...
  const data = await page.evaluate(async () => {
    ...
    if (
      document.querySelector(
        '#specificationSwatchControl > div > div.specification-swatch-control--item-purchase > div.item-purchase--item-availability-message.ng-binding > label.iteminstock > strong > span'
      )
    ) {
      if (
        document
